...
| **Change-IR / deletions (op-primary)** | ✅ | ✅ | derived-string | ✅ | disclosed | silent block-replace |
...
- **Op-primary vs derived (CCR-0110):** IOS, NX-OS, and EOS are **op-primary** — every removal/negation is a native `ChangeOp` on `ParsedConfig.change_ops`, and their legacy `no_commands` string channels are now empty (except five residual derived-only `ParsedConfig` tombstones: the whole-process `no router bgp`, the two OSPF area-type resets, and — added by CCR-0211's completion, c58904f — the two sub-block removals `field:policy_maps:{pm}:classes:{cls}` for `no class <name>` under a policy-map and `field:lines:{type}:{first}:access_class_{dir}` for `no access-class … in|out` under a line). IOS-XR, JunOS, and PAN-OS are **not** op-primary: IOS-XR still emits **derived tombstone strings** for its section/singleton removals (comms singletons `singleton:ntp` / `singleton:dns` stay string-only), while JunOS and PAN-OS do not process deletion semantics at all. Post-CCR-0210 those last two differ in what happens INSTEAD, which is the part that matters to a consumer: **JunOS discloses** — `delete` / `deactivate` / `activate` / `replace` / `rename` / `annotate` / `insert` / `copy` and the brace `inactive:` / `replace:` tags land in `unrecognized_blocks` and reach `CoverageArea.UNRECOGNIZED`, so a report says part of the change could not be read; **PAN-OS discloses nothing, and removes things anyway** — `_collect_unrecognized_blocks` returns `[]`, so nothing is ever surfaced, while a RESTATED block replaces its contents wholesale: a proposal restating the security rulebase without one of its rules silently drops that rule (security rules parse into `acls`, a whole-object-replace field), whereas omitting a block entirely removes nothing. Silent removal, no tombstone, no disclosure — the worst combination of the three parsers here. Simulating JunOS deletions is CCR-0214, deferred. IOS-XR's tombstone coverage is also narrower than IOS/NX-OS/EOS: removals it does not model (`no interface`, `no vlan`) are disclosed via CCR-0203 rather than applied.
